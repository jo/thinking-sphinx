...
module ThinkingSphinx
  ...
  class Search
    ...
    def excerpt_for(string, model = nil, options = {})
      if model.is_a? Hash
        options = model
        model = nil
      end
      ...
      populate
      index = indexes.split(',').first
      client.excerpts({
                        :docs   => [string],
                        :words  => results[:words].keys.join(' '),
                        :index  => index =~ /_(core|delta)$/ ? index : index + '_core'
                      }.merge(options)
      ).first
    end
    ...
    def add_excerpter
      each do |object|
        object.search = self if object.respond_to?(:search=)
